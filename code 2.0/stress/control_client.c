...
void display_help(int error)
{
  	printf("Usage:\n"
	       "client <options>\n\n"
	       "-s [NUM] --packetsize            set the packet size to be sent and received\n"
	       "-a       --mainaffinity=N        run main thread on processor #N\n"
	       "			 	 sending thread will run on the same processor\n,"
	       "				 if -b is not specified\n"
	       "-b       --threadaffinity=N      run sending thread on precessor #N\n"
	       "-y POLI  --policy=POLI           policy of realtime thread, POLI may be fifo(default) or rr\n"
               "                                 format: --policy=fifo(default) or --policy=rr\n"
	       "-p PRIO  --prio=PRIO             set priority of program's thread\n"
