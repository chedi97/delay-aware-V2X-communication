	...
	Sending the message to the server
      */
      ...
      (void)snprintf((char *)&pu8a_client_frame[u16_client_data_off],
                     data_size ,"%s %d %ld %d","raw_packet_test" , u32_i++ ,client_send.tv_nsec, 0);
     ...
     while (1)
       {
          ...
          */
          ...
          if (strcmp(sArr[0],string)>=0 && (atol(sArr[2]) >=  0)) {
            ...
            printf("\tC:%d Min:%ld Act:%ld Avg:%ld Max:%"PRIu64" | Rcv:%d PDR:%d       \r"
                   ,cycle,min_latency,network_latency,(long)avg/cycle,max_latency,atoi(sArr[3]),
                   (atoi(sArr[3])*100)/frame_size);   
            fflush(stdout);
            
            if(histogram) {
              /* put values in us */
              /*index_plot= (int)nearbyint(network_latency/1000);*/ // without tikz
              index_plot= (int)nearbyint(network_latency/10000);// for tikz 
